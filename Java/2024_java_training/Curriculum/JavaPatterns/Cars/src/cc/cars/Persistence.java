...
public class Persistence
{
    ...
    private static String DEFAULT_FILENAME = "Dealership";

    private String filename;

    /**
    Build the manager with a specific base filename for serialization.
    */
    ...
    /**
    Build a manager using the default base filename.
    */
    public Persistence ()
    {
    ...
    */
    ...
    /**
    Helper method to load a list of objects from a given file.
    */
    @SuppressWarnings("unchecked") // We're given the expected list-item type
    private <T> List<T> loadList (String filename, Class<T> cls)
    {
        ...
        {
            LOG.log (Level.SEVERE, "Couldn't load from " + filename, ex);
        }
    ...
    /**
    Helper method to save a list of objects to a given file.
    */
    private <T> void saveList (String filename, List<T> list)
    {
        ...
        {
            LOG.log (Level.SEVERE, "Couldn't write to " + filename, ex);
        }
    }
    ...
    */
    public List<Car> loadCars()
    {
        ...
        if (new File (filename + ".cars").exists ())
            result = loadList (filename + ".cars", Car.class);
        if (result != null)
            return result;
    ...
    */
    ...
    {
        List<UsedCar> result = null;
        if (new File (filename + ".used").exists ())
            result = loadList (filename + ".used", UsedCar.class);
        if (result != null)
            return result;
    ...
    */
    public List<Part> loadParts()
    {
        ...
        if (new File (filename + ".parts").exists ())
            result = loadList (filename + ".parts", Part.class);
        if (result != null)
            return result;
    ...
    /**
    Saves car inventory to file.
    */
    public void saveCars (List<? extends Car> list)
    {
        saveList (filename + ".cars", list);
    }

    /**
    Saves used-car inventory to file.
    */
    public void saveUsedCars (List<? extends UsedCar> list)
    {
        saveList (filename + ".used", list);
    }

    /**
    Saves parts inventory to file.
    */
    public void saveParts (List<? extends Part> list)
    {
        saveList (filename + ".parts", list);
    }
}
